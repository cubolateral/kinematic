...
impl AnimatorHandle {
    fn activate(&self) -> Option<AnimatorHandle> {
        let previous = self.context.active.replace(Some(self.clone()));
        self.sync_scene_time();
        previous
    }

    fn restore(&self, previous: Option<AnimatorHandle>) {
...
mod tests {
    ...
    fn chain_collects_tasks_scheduled_by_a_closure() {
        ...
        let state = animator.handle.state.borrow();
        assert_eq!(state.tasks.len(), 1);

        let Task::Chain(tasks) = &state.tasks[0] else {
    ...
    fn all_collects_tasks_scheduled_by_a_closure() {
        ...
        let Task::All(tasks) = &state.tasks[0] else {
    ...
    fn repeat_collects_tasks_scheduled_by_a_closure() {
        ...
        let Task::Repeat(repetitions, tasks) = &state.tasks[0] else {
